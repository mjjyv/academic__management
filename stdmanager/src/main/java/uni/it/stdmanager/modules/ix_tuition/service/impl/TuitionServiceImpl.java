...
import uni.it.stdmanager.modules.ix_tuition.repository.TuitionExemptionRepository;
...
public class TuitionServiceImpl implements TuitionService {
        ...
        private final TuitionExemptionRepository tuitionExemptionRepository;
        ...
        public StudentTuitionResponse calculateTuition(UUID studentId, UUID semesterId) {
                ...
                // Bảo vệ dữ liệu: Nếu học kỳ đã đóng, không tính toán lại để tránh xóa dữ liệu
                // lịch sử
                if (semester.getIsActive() == null || !semester.getIsActive()) {
                        return mapToTuitionResponse(studentTuition);
                }

                // Lấy đơn giá học phí theo KHÓA (Admission Year)
                Integer admissionYear = student.getAdmissionYear() != null ? student.getAdmissionYear() : 2025;

                TuitionFee newFee = tuitionFeeRepository
                                .findFirstByAdmissionYearAndFeeTypeAndIsActiveTrueOrderByEffectiveDateDesc(
                                                admissionYear, "NEW")
                                .orElseGet(() -> {
                                        // Fallback to courseYear if admissionYear match not found
                                        String courseYear = admissionYear.toString();
                                        return tuitionFeeRepository
                                                        .findFirstByCourseYearAndFeeTypeAndIsActiveTrueOrderByEffectiveDateDesc(
                                                                        courseYear, "NEW")
                                                        .orElseThrow(() -> new RuntimeException(
                                                                        "Chưa cấu hình đơn giá học phí cho khóa "
                                                                                        + admissionYear));
                                });

                TuitionFee retakeFee = tuitionFeeRepository
                                .findFirstByAdmissionYearAndFeeTypeAndIsActiveTrueOrderByEffectiveDateDesc(
                                                admissionYear, "RETAKE")
                                .orElse(newFee);
                ...
                // Tính toán Miễn giảm (Exemptions)
                BigDecimal totalExemption = BigDecimal.ZERO;
                List<uni.it.stdmanager.modules.ix_tuition.entity.TuitionExemption> exemptions = tuitionExemptionRepository
                                .findAllByStudentIdAndIsActiveTrue(studentId);

                for (var ex : exemptions) {
                        // Check semester range
                        if (ex.getStartSemester() != null && ex.getStartSemester().getId().equals(semesterId)) {
                                // Simplified check: if current semester matches start, or logic for range
                                BigDecimal exAmount = rawAmount
                                                .multiply(ex.getExemptionPercentage().divide(new BigDecimal(100)));
                                totalExemption = totalExemption.add(exAmount);
                        }
                }
                studentTuition.setExemptionAmount(totalExemption);

                // Calculate net amount
                BigDecimal netAmount = rawAmount
                                .subtract(studentTuition.getScholarshipDeduction() != null
                                                ? studentTuition.getScholarshipDeduction()
                                                : BigDecimal.ZERO)
                                .subtract(studentTuition.getExemptionAmount() != null
                                                ? studentTuition.getExemptionAmount()
                                                : BigDecimal.ZERO);
                ...
                // Calculate debt
                BigDecimal paidAmount = studentTuition.getPaidAmount() != null ? studentTuition.getPaidAmount()
                                : BigDecimal.ZERO;
        ...
        public TuitionSummaryResponse getDebtSummaryForCurrentStudent() {
                ...
                return TuitionSummaryResponse.builder()
                                .totalDebt(totalDebt)
                                .totalPaid(totalPaid)
                                .semesterTuitions(tuitions.stream().map(this::mapToTuitionResponse)
                                                .collect(Collectors.toList()))
                                .build();
        }
        ...
        public PaymentResponse processPayment(PaymentRequest request) {
                ...
                if (tuition.getDebtAmount().compareTo(BigDecimal.ZERO) <= 0) {
                        ...
                        List<CourseRegistration> registrations = courseRegistrationRepository
                                        .findAllByStudentIdAndCourseSectionSemesterId(tuition.getStudent().getId(),
                                                        tuition.getSemester().getId());
                        for (CourseRegistration reg : registrations) {
                                if (reg.getStatus() != 3) {
                                        reg.setIsPaid(true);
                                }
